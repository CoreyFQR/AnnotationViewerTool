...
2.0.1 使用全新的圆角显微镜图像标注 logo、PANTONE 蓝色系界面和圆角 IoU 阈值输入框，现有数据格式、快捷键与操作流程保持不变。
...
- 文件名搜索：可在左侧图片列表上方输入图片名、JSON 或 TXT 标注文件名的一部分，即时筛选结果；按 Enter 可跳转到结果列表，按 Esc 或点击 `清空` 可恢复完整列表。筛选只影响当前浏览和前后翻页，不影响标注统计、GT/Pred 对比统计和对比图导出。
- 标注统计：点击 `统计标注` 后，可按文件夹和类别统计图片数、标注文件数、标注格式和标注框数；类别行的图片数和标注文件数只计算实际包含该类别的图片与标注文件，总计行显示整个文件夹的总量。统计表支持复制和导出 CSV。
