...
用于查看图片标注框，支持 LabelMe JSON 和 YOLO TXT 两种标注格式，并提供标注统计、GT/Pred 对比、错误分析和对比图导出。
...
- 标注浏览：左侧图片列表用于切换图片，右侧显示图片、标注框和标注列表；支持显示/隐藏标签、缩放、适应窗口和刷新当前目录。
- 标注统计：点击 `统计标注` 后，可按文件夹和类别统计图片数、标注文件数、标注格式和标注框数；统计表支持复制和导出 CSV。
- GT / Pred 对比：在 `Pred labels` 中选择 YOLO 预测 `labels` 文件夹，设置 IoU 阈值后，可查看 GT 与 Pred 的匹配结果。
- 显示模式：支持 `仅显示GT`，也支持开启 `错误分析`，用 TP、FP、FN 区分正确预测、误检和漏检。
